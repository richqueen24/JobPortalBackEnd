...
export const forgotPassword = async (req, res) => {
    try {
        ...
        // Also clear any old token flow to avoid confusion
        user.resetPasswordToken = '';
        user.resetPasswordExpires = undefined;
        await user.save();

        // Send email with the code if SMTP configured
        try {
            const { sendMail } = await import('../utils/mailer.js');
            const subject = 'Your password reset code';
            const text = `Your password reset code is: ${code}. It will expire in 1 hour.`;
            const html = `<p>Your password reset code is: <strong>${code}</strong>. It will expire in 1 hour.</p>`;

...
export const scheduleInterview = async (req, res) => {
  try {
    ...
    const { applicationId, interviewTitle, startTime, duration } = req.body;
    ...
    try {
      ...
      if (applicant) {
        ...
        applicant.notifications.unshift({
          ...
          message: `An interview has been scheduled for ${application.job.title}: ${interviewTitle}`,
          data: { applicationId: application._id, meetingLink: meeting.meetingLink, startTime },
          read: false,
        });
      ...
      if (process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS) {
        ...
        if (applicant && applicant.email) {
          await transporter.sendMail({
            ...
            text: `Your interview for ${application.job.title} is scheduled at ${new Date(startTime).toLocaleString()}. Join: ${meeting.meetingLink}`,
            html: `<p>Your interview for <strong>${application.job.title}</strong> is scheduled at <strong>${new Date(startTime).toLocaleString()}</strong>.</p><p><a href="${meeting.meetingLink}">Join Meeting</a></p>`,
          });
        }
      }
    } catch (mailErr) {
    ...
    return res.status(200).json({ message: 'Interview scheduled', success: true, meeting });
  } catch (error) {

...
export const getConversationById = async (req, res) => {
  try {
    ...
    // If this conversation is attached to an application, include that id so the frontend can schedule interviews
    let applicationInfo = null;
    try {
      const { Application } = await import('../models/application.model.js');
      const app = await Application.findOne({ conversation: convoId }).select('_id job');
      if (app) applicationInfo = { applicationId: app._id, jobId: app.job };
    } catch (appErr) {
      console.error('Failed to lookup application for conversation', appErr);
    }

    return res.status(200).json({ conversation: convo, applicationInfo, success: true });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Server error", success: false });
  }
